# 项目需求文档 (PRD): CineFlow (影流) 通用视频生成流水线

## 1. 项目概述

本项目旨在开发一个高效的后端脚本程序，通过调用 Sora API 及未来扩展的多模型接口，根据预设的分镜脚本批量生成视频。程序支持多线程并发处理，能够扫描统一的 `input` 目录读取 JSON 格式的分镜数据，并将生成的视频及其相关素材规范、可靠地保存到 `output` 目录中。
