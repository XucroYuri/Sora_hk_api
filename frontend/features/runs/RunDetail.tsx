...
export const RunDetail: React.FC<{ id: string }> = ({ id }) => {
  ...
  const filteredTasks = tasks.filter(t => {
     if (filter === 'failed_retryable') {
         return (t.status === RunStatus.Failed || t.status === RunStatus.DownloadFailed) && Boolean(t.retryable);
     }
     return true;
  });

  const downloadMetadata = async (task: Task) => {
    try {
      const metadata = await api.getTaskMetadata(task);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
           ...
           >
             {t('filter.failed_retryable')} ({tasks.filter(t => (t.status === RunStatus.Failed || t.status === RunStatus.DownloadFailed) && Boolean(t.retryable)).length})
           </button>
           <button 
