...
export const SettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (activeTab === 'models') {
      api.getAdminModels().then(setModels);
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-100">
                   <tr>
                     ...
                     <th className="px-4 py-3">{t('col.capabilities')}</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {providers.map(provider => (
                    <tr key={provider.id} className="hover:bg-slate-50">
                      <td className="px-4 py-3 font-medium text-slate-800">{provider.display_name}</td>
                      <td className="px-4 py-3 text-slate-500">
                        <div>{provider.supports_image_to_video ? 'I2V' : '-'} / {provider.supports_pro ? 'Pro' : 'Std'}</div>
                        <div className="text-xs text-slate-400 mt-1">
                          {provider.supported_durations.join(', ')}s · {provider.supported_resolutions.map(res => res === 'horizontal' ? '16:9' : '9:16').join(' / ')}
                        </div>
                      </td>
