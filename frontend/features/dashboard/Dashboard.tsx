             ...
             <tbody className="divide-y divide-slate-100">
                {runs.slice(0, 5).map(run => (
                  <tr key={run.id} className="hover:bg-slate-50">
                    <td className="px-6 py-3 font-medium text-slate-900">{run.storyboard_name || run.storyboard_id || run.id}</td>
                    <td className="px-6 py-3"><StatusBadge status={run.status} label={t(`status.${run.status}` as any)} /></td>
                    <td className="px-6 py-3">
                       <div className="w-24 bg-slate-200 rounded-full h-1.5">
                         <div className="bg-blue-600 h-1.5 rounded-full" style={{ width: `${run.total_tasks ? (run.completed / run.total_tasks) * 100 : 0}%` }}></div>
                       </div>
                    </td>
