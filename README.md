...
2.  **🧩 多参考图矩阵拼接**:
    ...
    *   **角色一致性**: 深度适配 Sora 网页端的 Character 能力，通过在 Prompt 中嵌入固定的 **角色ID (Role ID)** 格式，实现多镜头下的角色形象统一。
4.  **🔌 多服务商与多模型聚合**:
...
5.  **📝 LLM 剧本预处理模块**:
    *   计划引入 LLM (Large Language Model) 辅助模块，实现从“自然语言剧本/小说”到“标准化分镜 JSON”的自动化转换与拆解。
6.  **🖥️ 跨平台 GUI 客户端 (待定)**:
