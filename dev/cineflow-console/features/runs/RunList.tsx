            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={run.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium">{run.storyboard_name}</td>
                  <td className="px-6 py-4"><StatusBadge status={run.status} label={t(`status.${run.status}` as any)} /></td>
                  <td className="px-6 py-4">
                     <div className="flex items-center gap-2">
                       <div className="w-24 bg-slate-200 rounded-full h-1.5 overflow-hidden">
                         <div className="bg-blue-600 h-1.5" style={{ width: `${(run.completed / run.total_tasks) * 100}%` }}></div>
                       </div>
                       <span className="text-xs text-slate-500">{Math.round((run.completed / run.total_tasks) * 100)}%</span>
                     </div>
                  </td>
                  <td className="px-6 py-4">
